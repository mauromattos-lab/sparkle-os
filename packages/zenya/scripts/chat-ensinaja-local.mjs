#!/usr/bin/env node
// REPL terminal pra conversar com o agente Ensinaja — bypass do banco.
...
//
// Diferença do HL REPL: sem integração externa real. Ensinaja não tem integração custom —
...
// Uso:
//   cd packages/zenya
//   node --env-file=.env scripts/chat-ensinaja-local.mjs
//
...
function banner() {
  console.log(`\n🎓 REPL Ensina Já Rede de Educação (Zenya)`);
...
function prompt() {
  rl.question('você > ', async (input) => {
    ...
    try {
      ...
      console.log(`\nzenya> ${result.text}  (${elapsed}s · ${stepTrail.length} step${stepTrail.length === 1 ? '' : 's'})\n`);
    } catch (err) {
      console.error(`\n❌ erro: ${err.message}\n`);
    }
