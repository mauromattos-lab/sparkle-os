...
function buildN8nStatusHtml(online: boolean, workflowCount: number | null, error: string | null): string {
  if (!online) {
    return `
      <div class="integration-card offline">
        <div class="integration-name">
          <span class="status offline"><span class="status-dot"></span>n8n &mdash; Offline</span>
        </div>
        <div class="alert-item" style="margin-top:12px;">
          <div class="alert-title">n8n indisponivel</div>
          <div class="alert-desc">${error ?? 'Servico nao responde.'}</div>
          <div class="alert-sop">
            <strong>O que verificar:</strong>
            Acesse a VPS onde o n8n esta hospedado e confirme que o servico esta rodando.
            ...
            Configure a variavel <code>N8N_URL</code> no <code>.env</code> com a URL correta (VPS ou tunnel SSH).
          </div>
        </div>
      </div>
    `;
  ...
  const workflowLine =
    workflowCount !== null
      ...
      : `<div class="integration-detail" style="color:#a0aec0;">Contagem de workflows indisponivel &mdash; configure N8N_API_KEY para habilitar</div>`;

  return `
    <div class="integration-card ok">
      <div class="integration-name">
        <span class="status ok"><span class="status-dot"></span>n8n &mdash; Operacional</span>
      </div>
      ${workflowLine}
    </div>
  `;
...
function buildStoryProgressHtml(done: number, inProgress: number, inReview: number, ready: number, draft: number, total: number): string {
  if (total === 0) {
  ...
  return `
    <div class="cards">
      <div class="card">
        <div class="card-label">Concluidas</div>
      ...
      <div class="card">
        <div class="card-label">Em Revisao</div>
...
zenyaRouter.get('/', async (c) => {
  ...
  const n8nHtml = buildN8nStatusHtml(zenyaStatus.online, zenyaStatus.workflowCount, zenyaStatus.error);
  const storyHtml = buildStoryProgressHtml(
    epicProgress.done, epicProgress.inProgress, epicProgress.inReview,
    epicProgress.ready, epicProgress.draft, epicProgress.total,
  );

  const content = `
    <div class="main-header">
      <h1>Nucleo Zenya</h1>
      <div class="subtitle">Status operacional da Zenya &mdash; verificado em ${checkedAt}</div>
    </div>
    <div class="alert-section">
      <h2>Conectividade n8n</h2>
      <div class="integration-grid">${n8nHtml}</div>
    </div>
    <div class="alert-section">
      <h2>Progresso das Stories &mdash; Epic 2 (Zenya)</h2>
      ${storyHtml}
    </div>
  `;
