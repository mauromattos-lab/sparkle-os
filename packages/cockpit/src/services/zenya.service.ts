 ...
 * Env vars are read at call time (not module load time) to support test isolation.
 */
export async function getZenyaStatus(): Promise<ZenyaStatus> {
  ...
  try {
    ...
    return {
      online: false,
      workflowCount: null,
      error: 'n8n nao responde. Verifique se o servico esta rodando na VPS.',
    };
  }
  ...
  if (!online) {
    return {
      online: false,
      workflowCount: null,
      error: 'n8n respondeu com erro. Verifique o status do servico na VPS.',
    };
  }
